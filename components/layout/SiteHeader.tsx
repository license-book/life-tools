...
export default function SiteHeader() {
  ...
  >
    ...
    <nav id="mobile-category-menu" className={`${styles.mobileNav}${menuOpen?` ${styles.open}`:""}`} aria-label="모바일 주요 카테고리"><div className={`container ${styles.mobileNavInner}`}><div className={styles.mobileSearch}><ToolSearch compact onNavigate={()=>setMenuOpen(false)}/></div>{nav.map((item)=><Link key={item.code} href={item.href} onClick={()=>setMenuOpen(false)}><strong>{item.label}</strong><span>{item.description}</span></Link>)}</div></nav>
  </header>;
}
