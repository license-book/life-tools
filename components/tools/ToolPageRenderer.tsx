    ...
    <>
      ...
      <main>
        ...
        {tool.sections.map((section) => (
          <section className="section" key={section.title}>
            <div className="container">
              ...
              {section.cards?.length ? <div className="grid">{section.cards.map((card) => <article className="card" key={card.title}>{card.label ? <span className="category-label">{card.label}</span> : null}<h3>{card.title}</h3><p>{card.description}</p></article>)}</div> : null}
              {section.paragraphs?.length ? <div className="card">{section.paragraphs.map((paragraph, index) => <p key={`${section.title}-${index}`} style={index > 0 ? { marginTop: 16 } : undefined}>{paragraph}</p>)}</div> : null}
        ...
        {tool.freeResources?.length ? (
          <section className="section"><div className="container"><h2>무료 결과물</h2><p className="section-intro">계산 결과를 화면에서 확인하는 데서 끝내지 않고 실제 계획과 비교에 활용할 수 있습니다.</p><div className="grid">{tool.freeResources.map((resource) => <article className="card" key={resource.title}><span className="category-label">{resource.type}</span><h3>{resource.title}</h3><p>{resource.description}</p></article>)}</div></div></section>
        ) : null}
