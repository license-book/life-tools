...
export default function LoanCalculator() {
  ...
  const balanceTrend = useMemo(() => {
    const points = result.schedule
      .filter((row) => row.month === 1 || row.month % 12 === 0 || row.month === result.schedule.length)
      .map((row) => ({ label: row.month === 1 ? "1개월" : `${Math.ceil(row.month / 12)}년`, value: row.balance }));
    return points.length > 14
      ? points.filter((_, i) => i === 0 || i === points.length - 1 || i % Math.ceil(points.length / 12) === 0)
      : points;
  }, [result]);
  ...
  const exportCsv = () => {
    ...
    const rows = result.schedule
      .map((row) => [row.month, row.payment, row.principal, row.interest, row.balance].join(","))
      .join("\n");
    const summary = `대출금액,${applied.principal}\n연이자율,${applied.annualRate}%\n대출기간,${months}개월\n상환방식,${methodLabel[applied.method]}\n총상환액,${result.totalPayment}\n총이자,${result.totalInterest}\n\n`;
    const blob = new Blob(["\uFEFF" + summary + header + rows], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `loan-repayment-plan-${applied.method}.csv`;
    anchor.click();
    ...
    <div className="tool-layout">
      ...
      <section className="panel print-summary" aria-live="polite">
        ...
        <div className="action-row no-print" style={{ gridTemplateColumns: "repeat(3,minmax(0,1fr))" }}>
          ...
          <ToolOutputActions embedded targetSelector=".tool-layout" fileName={`대출상환계산-${methodLabel[applied.method]}`} />
        </div>
      ...
      <section className="panel full-width comparison-panel">
        <div className="section-heading-row">
          <div><span className="category-label">STEP 3 · 자동 비교</span><h2>원리금균등 vs 원금균등</h2></div>
      ...
      <section className="panel full-width repayment-plan">
        <div className="section-heading-row">
          <div><span className="category-label">STEP 4 · 무료 결과물</span><h2>{methodLabel[applied.method]} 월별 상환계획표</h2></div>
          <span className="print-only print-meta">대출 {won.format(applied.principal)}원 · 연 {applied.annualRate}% · {months}개월</span>
        </div>
