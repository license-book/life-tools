...
export default function HomePage() {
  ...
  return <><SiteHeader/><main className={styles.main}>
    ...
    <section id="categories" className={styles.sectionAlt}><div className="container"><div className={styles.headingRow}><div className={styles.headingBlock}><h2>카테고리별 생활도구</h2><p>필요한 분야부터 빠르게 찾아보세요.</p></div></div><div className={styles.categoryGrid}>{categories.map((item)=><Link className={styles.categoryCard} key={item.code} href={item.href}><span className={styles.iconWrap}><LineIcon name={item.icon}/></span><span className={styles.categoryCode}>{item.code} · {categoryTools[item.code].length}</span><h3>{item.title}</h3><p>{item.description}</p><span className={styles.categoryArrow}>→</span></Link>)}</div></div></section>
